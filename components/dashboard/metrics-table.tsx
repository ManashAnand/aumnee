...
import { formatVelocity, getVelocityColor, getVelocityBadgeColor } from '@/lib/dashboard-utils';
              ...
              {metrics.map((metric, index) => (
                <tr key={metric.name} className={`border-b border-gray-100 ${index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}`}>
                  <td className="py-4 px-4">
                    <div className="font-medium text-gray-900 capitalize">{metric.name}</div>
                  </td>
